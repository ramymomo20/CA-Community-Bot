...
class MoreOptionsView(View):
    ...
    async def clear_position_callback(self, interaction: discord.Interaction):
        ...
        state = await init_state(interaction.guild_id, interaction.channel.id)
        team = state["teams"][self.team_number - 1]
        
        # Show position selection for clearing
        positions_with_players = [(pos, data) for pos, data in team.items() if data is not None]
        ...
        for pos, data in positions_with_players:
            member = data['player']
            button = Button(
            ...
            async def make_callback(pos=pos):
                async def callback(i: discord.Interaction):
                    await i.response.defer(ephemeral=True)
                    
                    # Get a fresh copy of state to modify
                    state_copy = dict(get_channel_state(i.channel_id))
                    if not state_copy: return
                    
                    team_copy = state_copy["teams"][self.team_number - 1]
                    
                    # Try to move a sub to this position
                    moved_sub = await move_sub_to_position(state_copy, pos, self.team_number)
                    if moved_sub:
                        await i.followup.send(f"✅ Moved {moved_sub.mention} from subs to {pos}", ephemeral=True)
                    else:
                        team_copy[pos] = None
                        await i.followup.send(f"✅ Cleared {pos} position", ephemeral=True)
                    
                    update_state(i.channel_id, state_copy)
        ...
        asyncio.create_task(delete_after_delay(interaction, 10))  # Give more time for position selection
    ...
    async def clear_lineup_callback(self, interaction: discord.Interaction):
        ...
        for team in state_copy["teams"]:
            for pos in team:
                team[pos] = None
        # Clear subs list as well
        state_copy["subs"].clear()
        update_state(interaction.channel_id, state_copy)
    ...
    async def highlight_callback(self, interaction: discord.Interaction):
        ...
        # enforce the same 10-min cooldown
        can_use, minutes_left = check_notification_cooldown(interaction.channel_id)
        if not can_use:
            return await interaction.followup.send(
                f"❌ Highlight is on cooldown. Try again in {minutes_left} minute(s).",
                ephemeral=True
            )
        ...
        # confirm
        await interaction.followup.send("✅ Highlighted successfully!", ephemeral=True)

        # auto-remove the confirmation
        asyncio.create_task(delete_after_delay(interaction))
    ...
    if initiator_full and opponent_full and len(ready_list) >= initiator_signed_count:
        ...
        for i in range(num_teams_for_match):
            if i < len(state_copy["teams"]):
                is_full, count = await check_lineup_readiness(state_copy["teams"][i], positions_to_check, game_type)
