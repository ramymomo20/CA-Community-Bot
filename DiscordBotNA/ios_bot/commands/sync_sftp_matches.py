...
@commands.slash_command(name="sync_sftp_matches", description="Find SFTP JSONs missing from DB (admin)")
...
async def _run_sync(ctx, days, dry_run, limit):
    ...
    for fname in files:
        checked += 1
        canonical = filename_to_match_id(fname)
        row = await db.pool.fetchrow("SELECT id FROM MATCH_STATS WHERE match_id = $1", canonical)
        if not row:
