...
--
-- STATUS: PROPOSED, NOT YET APPLIED. Pasted into a Claude review session on
-- 2026-08-26 for evaluation before any migration attempt. A prior migration
-- attempt (via a different AI tool) against this target failed -- lost
-- functionality, data type issues, hub site would not disable. See
-- docs/canonical_schema_deployment.md and the project's engineering-audit
-- memory for context. Do not apply this file directly against production.
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_team_membership_single_active_non_captain
    ON core.team_memberships(account_id)
    WHERE left_at IS NULL
      AND is_active = TRUE
      AND membership_role <> 'captain';
