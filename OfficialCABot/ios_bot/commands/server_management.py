...
@bot.slash_command(
    name="set_server_ingame_name",
    description="Record the exact serverName a server reports in match JSON (admin only)."
)
async def set_server_ingame_name(
    ctx,
    server: str = Option(description="The server's admin-facing name (as shown in /edit_servers)", required=True),
    in_game_name: str = Option(description="The exact matchInfo.serverName string from that server's match JSON, e.g. '[IOSCA] Atlanta GA'", required=True),
):
    if not ctx.author.guild_permissions.administrator:
        await ctx.respond("❌ You need administrator permissions to use this command.", ephemeral=True)
        return

    server_row = await bot.db.servers.get_server_by_name(server)
    if not server_row:
        await ctx.respond(f"❌ No server found named '{server}'. Check `/edit_servers edit` for the exact name.", ephemeral=True)
        return

    ok = await bot.db.servers.set_ingame_server_name(server_row["id"], in_game_name)
    if ok:
        await ctx.respond(
            f"✅ '{server}' will now be matched against match JSON reporting `matchInfo.serverName = \"{in_game_name}\"`.",
            ephemeral=True,
        )
    else:
        await ctx.respond("❌ Failed to save. Check the logs for details.", ephemeral=True) 