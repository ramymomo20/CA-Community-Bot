...
async def delete_team_command(ctx: ApplicationContext, team_name: Option(str, "Admin only: Specify team name to delete", required=False)):
    ...
    await ctx.respond(
        f"⚠️ **Warning:** Are you sure you want to delete the team '{target_guild_name}'? "
        "This deactivates the team and frees its roster players to join another team -- "
        "an admin can reactivate it later if needed, but it won't show up anywhere until then.",
        view=view,
        ephemeral=True,
    )
...
@bot.slash_command(
    name="reactivate_team",
    description="[ADMIN] Re-enable a previously deleted (deactivated) team."
)
@commands.has_permissions(administrator=True)
async def reactivate_team_command(
    ctx: ApplicationContext,
    team_name: Option(str, "Name of the deactivated team to reactivate", required=True),
):
    await ctx.defer(ephemeral=True)

    # get_team_by_name queries IOSCA_TEAMS directly with no is_active filter,
    # so it finds deactivated teams too (unlike get_all_teams*, which only
    # list active ones).
    team = await bot.db.teams.get_team_by_name(team_name)
    if not team:
        await ctx.followup.send(
            f"Could not find a deactivated team named '{team_name}'. Use the exact registered name.",
            ephemeral=True,
        )
        return
    if team.get("is_active", True):
        await ctx.followup.send(f"'{team_name}' is already active.", ephemeral=True)
        return

    ok = await bot.db.teams.reactivate_team(team["guild_id"])
    if ok:
        await ctx.followup.send(f"✅ Team '{team_name}' has been reactivated.", ephemeral=True)
    else:
        await ctx.followup.send(f"❌ Failed to reactivate '{team_name}'.", ephemeral=True) 
