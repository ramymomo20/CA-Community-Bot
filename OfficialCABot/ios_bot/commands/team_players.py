...
def _vice_captain_ids(team_data: dict) -> list:
    """vice_captain_ids is stored as a JSONB list of raw Discord ids."""
    raw = team_data.get('vice_captain_ids')
...
def _is_captain_or_vice_captain(team_data: dict, user_id: int) -> bool:
    return user_id == team_data.get('captain_id') or user_id in _vice_captain_ids(team_data)
...
class RegisterPlayersView(View):
    def __init__(self, author_id, guild, team_data):
        ...
        self.all_current_player_ids_on_team = bot.db.teams.get_unique_player_ids(self.team_data.get('players', []))
        if self.team_data.get('captain_id'):
            self.all_current_player_ids_on_team.add(self.team_data['captain_id'])
        vice_captain_ids = self.team_data.get('vice_captain_ids') or []
        if isinstance(vice_captain_ids, str):
            vice_captain_ids = json.loads(vice_captain_ids) if vice_captain_ids else []
        for vc_id in vice_captain_ids:
            self.all_current_player_ids_on_team.add(vc_id)
    ...
    async def confirm_callback(self, interaction: discord.Interaction):
        ...
        # Check 8-player limit
        if current_player_count_on_team + num_potential_new_players > 8:
            slots_available = 8 - current_player_count_on_team
            if slots_available <= 0:
                await interaction.response.edit_message(content="The team roster is already full (8 unique players). No new players can be added.", view=None)
            else:
                await interaction.response.edit_message(content=f"You are trying to add {num_potential_new_players} new player(s), but the team only has {slots_available} slot(s) available to reach the 8 unique player limit. Please revise your selection.", view=None)
            self.stop()
            return

        # Add players directly to team -- one read-modify-write for the
        # whole batch instead of one round trip per selected player.
        try:
            ...
            for player_id in ui_selected_ids:
                ...
                if player_id in potential_new_additions_ids:
                    name_by_id[player_id] = member.display_name
                    new_players_payload.append({
            ...
            result = await bot.db.teams.add_players_to_team(self.guild.id, new_players_payload)
            newly_added_names = [name_by_id[pid] for pid in result.get("added", []) if pid in name_by_id]
...
async def register_players(ctx: ApplicationContext):
    ...
    if not _is_captain_or_vice_captain(team_data, ctx.user.id):
...
async def remove_player(ctx: ApplicationContext, player: Option(Member, "Select the player to remove")):
    ...
    vice_captain_ids = _vice_captain_ids(team_data)
    if not _is_captain_or_vice_captain(team_data, ctx.user.id):
    ...
    if player.id == captain_id:
        ...
        return
    if player.id in vice_captain_ids:
